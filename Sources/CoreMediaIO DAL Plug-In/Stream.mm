...
#import "VirtualCameraComposer-Swift.h"

@interface Stream () <VideoComposerDelegate> {
...
@property VideoComposer *videoComposer;
...
- (void)dealloc {
    ...
    [self.videoComposer stopRunning];
    self.videoComposer = nil;
}

- (void)startServingFrames {
    self.videoComposer = [[VideoComposer alloc] init];
    self.videoComposer.delegate = self;
    [self.videoComposer startRunning];
}
...
- (void)videoComposer:(VideoComposer *)composer didComposeImageBuffer:(CVImageBufferRef)imageBuffer {
